...
fun DocumentBar(
    ...
    Column(
        ...
        modifier = modifier.background(MaterialTheme.colorScheme.surfaceContainer.copy(alpha = 0.92f))
    ) {
        Box(
            if (isLandscape)
                Modifier
        ...
        val color = MaterialTheme.colorScheme.surfaceContainerHigh.copy(alpha = 0.97f)
        if (isLandscape) {
            Column(
                horizontalAlignment = Alignment.CenterHorizontally,
                modifier = Modifier
